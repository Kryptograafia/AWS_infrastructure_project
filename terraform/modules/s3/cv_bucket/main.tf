...
# S3 bucket notification to trigger Textract Lambda on CV uploads
resource "aws_s3_bucket_notification" "cv_upload_notification" {
  bucket = aws_s3_bucket.cv_upload_bucket.id

  lambda_function {
    lambda_function_arn = var.textract_lambda_arn
...
# Permission for S3 to invoke the Textract Lambda function
resource "aws_lambda_permission" "allow_s3_invoke" {
  statement_id  = "AllowExecutionFromS3Bucket"
  action        = "lambda:InvokeFunction"
  function_name = var.textract_lambda_function_name
  principal     = "s3.amazonaws.com"
  source_arn    = aws_s3_bucket.cv_upload_bucket.arn
}